<template>
    <v-row justify="center">
        <v-col cols="12" sm="8">
            <div class="title py-4">List of mature plants grouped by volume descending</div>
            <v-data-table
                    :headers="headers"
                    :items="items"
                    :items-per-page="20"
            ...
            >
                <template v-slot:top>
                    <v-toolbar flat color="white">
                        <v-toolbar-title>Mature plants group by type</v-toolbar-title>
                        <v-divider
                                class="mx-4"
                                inset
                                vertical
...
<script>
    ...
    export default {
        ...
        methods:{
            getReport() {
                this.loading = true
                axios.get(`${config.apiUrl}/report/mature-plant`).then(res => {
                    this.items = res.data
                }).finally(() => {
                    this.loading = false
                })
